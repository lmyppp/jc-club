...
public class StpInterfaceImpl implements StpInterface {
    ...
    public List<String> getPermissionList(Object loginId, String loginType) {
        // 返回此 loginId 拥有的权限列表
        /*
        获取用户的登录信息。怎么获取，如下三种方案
            1、从数据库获取
            2、从redis获取
            3、redis + 数据库 （需要解决缓存和数据库 数据一致的问题）
         */
        // 从redis中获取权限
    ...
    private List getAuth(String loginId, String prefix) {
        ...
        if (authRolePrefix.equals(prefix)) {
            ...
            authList = roleList.stream().map(authRole -> authRole.getRoleKey()).collect(Collectors.toList());
            ...
            authList = permissionList.stream().map(authPermission -> authPermission.getPermissionKey()).collect(Collectors.toList());
        }
        return authList;
    }
